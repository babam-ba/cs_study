...
props로부터 파생된 state를 가져온다는 뜻으로 `props`로 받아온 것을 `state`에 넣어주고 싶을 때 사용. Props강가 변경됐을 때, 상태도 변경해주기 위해 사용한다.
...
### 4. componentDidMount
컴포넌트 렌더링을 마치고 나서 호출되는 메서드. 이 메서드가 호출되는 시점에는 컴포넌트가 화면에 나타난 상태에다. side effect를 발생시키는 작업(DOM을 사용하는 경우, ajax 요청 등)을 하면 된다.(useEffect와 비슷한 역할)
